...
template <psl::HasEnumLogicalOps T>
	requires(std::is_same_v<bool, std::underlying_type_t<T>>)
constexpr bool enum_true(T const& a) noexcept {
	using I = std::underlying_type_t<T>;
...
template <psl::HasEnumLogicalOps T>
	requires(std::is_same_v<bool, std::underlying_type_t<T>>)
constexpr bool enum_false(T const& a) noexcept {
	using I = std::underlying_type_t<T>;
	return !static_cast<I>(a);
}
...
[[nodiscard]] constexpr T operator+(T const a, Y const b) noexcept
	requires requires(std::underlying_type_t<T> a, Y b) { a + b; }
{
	using I = std::underlying_type_t<T>;
