...
namespace psl
{
	...
	constexpr auto random_range(auto& generator, T min = std::numeric_limits<T>::min(),
	...
	template <typename T, typename Rt = T>
	inline auto random_range(T min = std::numeric_limits<T>::min(), T max = std::numeric_limits<T>::max()) noexcept
		-> Rt
	{
		PSL_EXCEPT_IF(min >= max, std::logic_error, "random_range max needs to be larger than min");
