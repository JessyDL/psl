...
inline namespace details {
	...
	template <typename FN, typename Opt, typename... Args>
	struct is_opt_invocable_r : std::conditional_t<std::is_invocable_v<FN, Opt, Args...>,
...
class optional {
  public:
	...
	explicit constexpr optional(value_init_t) noexcept(std::is_nothrow_constructible_v<T>)
		requires(std::is_constructible_v<T>)
		: m_ValueStorage({}) {}
	explicit constexpr optional(default_init_t) noexcept(std::is_nothrow_default_constructible_v<T>)
		requires(std::is_default_constructible_v<T>)
		: m_ValueStorage() {}
	explicit constexpr optional(zero_init_t) noexcept(std::is_nothrow_constructible_v<T, zero_init_t>)
		requires(std::is_constructible_v<T, zero_init_t>)
		: m_ValueStorage(zero_init) {}

	template <typename... Args>
		requires(disable_perfect_forward_illegal_type_v<optional, Args...>)
	...
	constexpr auto then(FN&& fn, Args&&... args) -> optional<is_opt_invocable_r_t<FN, optional, Args...>>
		requires(!IsOptional<is_opt_invocable_r_t<FN, optional, Args...>>)
	{
	 ...
	 */
	template <typename FN, typename... Args>
	constexpr auto then(FN&& fn, Args&&... args) -> is_opt_invocable_r_t<FN, optional, Args...>
		requires(IsOptional<is_opt_invocable_r_t<FN, optional, Args...>>)
	{
		return fn(*this, std::forward<Args>(args)...);
	}
	 ...
	 */
	template <typename FN, typename... Args>
	constexpr auto then(FN&& fn, Args&&... args) -> optional<is_opt_invocable_r_t<FN, T, Args...>>
		requires(!IsOptional<is_opt_invocable_r_t<FN, T, Args...>> && !is_opt_invocable_v<FN, optional, Args...> &&
				 std::is_default_constructible_v<T>)
	{
		if(m_ValueStorage)
			return {fn(m_ValueStorage.get(), std::forward<Args>(args)...)};
	 ...
	 */
	template <typename FN, typename... Args>
	constexpr auto then(FN&& fn, Args&&... args) -> is_opt_invocable_r_t<FN, T, Args...>
		requires(IsOptional<is_opt_invocable_r_t<FN, T, Args...>> && !is_opt_invocable_v<FN, optional, Args...> &&
				 std::is_default_constructible_v<T>)
	{
		if(m_ValueStorage)
