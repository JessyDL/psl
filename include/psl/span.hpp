...
namespace psl
{
	...
	{
		template <typename T, size_t Extent, i64 Stride = 1>
		class static_span
		{
		  public:
			static constexpr auto EXTENT{Extent};
			static constexpr auto STRIDE{Stride};
			...
			constexpr static_span(IsIterator auto begin) noexcept : m_Begin(&*begin){};
			constexpr static_span(const T* begin) noexcept : m_Begin(const_cast<T*>(begin)) {}

			template <size_t OtherExtent, i64 OtherStride>
			constexpr static_span(const static_span<T, OtherExtent, OtherStride>& other) noexcept : m_Begin(other.data())
			{}
			...
			constexpr static_span<T, Count, Stride> sub_span(size_type offset) const noexcept
			{
				return static_span<T, Count, Stride>{&*(begin()[offset * Stride])};
			}

			template <size_t Offset, size_t Count>
			constexpr static_span<T, Count, Stride> sub_span() const noexcept
			{
				return static_span<T, Count, Stride>{&*(begin()[Offset * Stride])};
			}

			constexpr static_span<T, Extent, -Stride> reverse() const noexcept
			{
				return static_span<T, Extent, -Stride>{m_Begin + ((Extent - 1) * Stride)};
			}
		...
		class dynamic_span
		{
		  public:
			...
			using reverse_iterator		 = contiguous_range_iterator<value_type, -Stride>;
			using const_reverse_iterator = contiguous_range_iterator<const value_type, -Stride>;
			constexpr dynamic_span() noexcept	 = default;
			constexpr dynamic_span(IsIterator auto begin, size_type count) noexcept
				: m_Begin(&*begin), m_End(m_Begin + (count * Stride)){};
			constexpr dynamic_span(const T* begin, size_type count) noexcept
				: m_Begin(const_cast<T*>(begin)), m_End(m_Begin + (count * Stride)){};

			template <i64 OtherStride>
			constexpr dynamic_span(const dynamic_span<T, OtherStride>& other) noexcept
			...
			constexpr dynamic_span<T, -Stride> reverse() const noexcept
			{
				return dynamic_span<T, -Stride>{m_End - Stride, size()};
			}
	...
	using span = std::conditional_t<Extent == dynamic_extent, dynamic_span<T, Stride>, static_span<T, Extent, Stride>>;
	...
	namespace _priv
	{
		...
		struct is_span<static_span<T, Extent, Stride>> : std::true_type
		{
		...
		template <typename T, i64 Stride>
		struct is_span<dynamic_span<T, Stride>> : std::true_type
		{
