...
namespace _priv {
	...
	template <typename Exception, typename... Ts>
		requires std::is_same_v<std::conditional_t<config::exceptions, void, int>, void>
	constexpr void throw_if_needed(bool condition, Ts&&... args) noexcept(false) {
		throw_exception<Exception, Ts...>(condition, std::forward<Ts>(args)...);
	}

	template <typename Exception, typename... Ts>
		requires std::is_same_v<std::conditional_t<config::exceptions, void, int>, int>
	constexpr void throw_if_needed(bool condition, Ts&&... args) noexcept(true) {
...
#define PSL_CONTRACT_EXCEPT_IF(expr, ...)                                                                              \
	if constexpr(psl::config::implementation_exceptions) {                                                             \
		if(!!(expr))                                                                                                   \
			throw psl::implementation_error(__VA_ARGS__);                                                              \
	} else {                                                                                                           \
		if(std::is_constant_evaluated() && !!(expr))                                                                   \
			throw psl::implementation_error(__VA_ARGS__);                                                              \
	}                                                                                                                  \
