...
namespace psl
{
	...
	inline namespace details
	{
		...
		struct crc32_t
		{
			...
			{
				return crc32_t<size, idx + 1>::generate(str,
														(prev_crc >> 8) ^ crc32_table[(prev_crc ^ str[idx]) & 0xFF]);
			}
			static constexpr uint32_t generate(const char8_t* str, uint32_t prev_crc = 0xFFFFFFFF)
			{
				return crc32_t<size, idx + 1>::generate(str,
														(prev_crc >> 8) ^ crc32_table[(prev_crc ^ str[idx]) & 0xFF]);
			}
		...
		struct crc32_t<size, size, dummy>
		{
			static constexpr uint32_t generate([[maybe_unused]] const char* str, uint32_t prev_crc = 0xFFFFFFFF)
			{
				return prev_crc ^ 0xFFFFFFFF;
			}
			static constexpr uint32_t generate([[maybe_unused]] const char8_t* str, uint32_t prev_crc = 0xFFFFFFFF)
			{
				return prev_crc ^ 0xFFFFFFFF;
			}
		};
		...
		struct crc64_t
		{
			...
			{
				return crc64_t<size, idx + 1>::generate(str,
														(prev_crc >> 8) ^ crc64_table[(prev_crc ^ str[idx]) & 0xFF]);
			}
			static constexpr uint64_t generate(const char8_t* str, uint64_t prev_crc = 0xFFFFFFFFFFFFFFFF)
			{
				return crc64_t<size, idx + 1>::generate(str,
														(prev_crc >> 8) ^ crc64_table[(prev_crc ^ str[idx]) & 0xFF]);
			}
		...
		// This is the stop-recursion function
		template <int size, class dummy>
		struct crc64_t<size, size, dummy>
		{
			...
			static constexpr uint64_t generate([[maybe_unused]] const char8_t* str,
											   uint64_t prev_crc = 0xFFFFFFFFFFFFFFFF)
			{
				return prev_crc ^ 0xFFFFFFFFFFFFFFFF;
			}
		};
	...
	constexpr uint32_t crc32(char const (&str)[length])
	{
		return crc32_t<length - 1>::generate(str);
	}
	...
	/// \returns 32 bit unsigned integer containing the crc32 value.
	template <size_t length>
	constexpr uint32_t crc32(char const* str)
	{
		return crc32_t<length - 1>::generate(str);
	}
	...
	template <size_t length>
	constexpr uint64_t crc64(char const (&str)[length])
	{
		return crc64_t<length - 1>::generate(str);
	}
	...
	/// \returns 64 bit unsigned integer containing the crc64 value.
	template <size_t length>
	constexpr uint64_t crc64(char const* str)
	{
		return crc64_t<length - 1>::generate(str);
	}

	template <psl::fixed_string Name>
	constexpr uint64_t crc64()
	{
		return crc64_t<Name.size()>::generate(static_cast<psl::string_view>(Name).data());
	}

	/// \brief crc64 checksum calculator for the input characters.
	/// \param[in] str the characters to calculate the crc64 for.
	/// \returns 64 bit unsigned integer containing the crc64 value.
	constexpr uint64_t crc64(psl::string_view str)
	{
	...
	constexpr uint64_t crc64(psl::pstring_view str)
	{
		uint64_t crc = 0xFFFFFFFFFFFFFFFF;
		for(uint8_t byte : str)
		{
	...
	/// \param[in] str the characters to calculate the crc32 for.
	/// \returns 32 bit unsigned integer containing the crc32 value.
	constexpr uint32_t crc32(psl::string_view str)
	{
